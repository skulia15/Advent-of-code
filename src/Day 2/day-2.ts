...
const GetFriendlyScore = (play: string) => {
  return 1 + Object.values(FriendlyPlayValue).indexOf(play as FriendlyPlayValue);
}

const GetGamesFromInput = () => {
  const result = readFileSync('./puzzle-input.txt', 'utf-8')
  const resultSplit = result.split('\r\n\r\n')
  const p = resultSplit.map(x => {
    return x.split('\r\n')
  })
  return p[0]
}
...
const GetGameOutcome = (enemyPlay: string, friendlyPlay: string) => {
  const friendlySameValueCheck = Object.values(FriendlyPlayValue).indexOf(friendlyPlay as FriendlyPlayValue);
  const enemySameValueCheck = Object.values(EnemyPlayValue).indexOf(enemyPlay as EnemyPlayValue);
